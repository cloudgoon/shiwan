...
$(document).ready(function() {
	$('#usersTasksItemTable').bootstrapTable({
		  ...
		  //请求方法
                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该用户任务记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/game/admin/usersTasksItem/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jp.getDictLabel(${fns:toJson(fns:getDictList('tasks_state'))}, value, "-");
		        }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#usersTasksItemTable').bootstrapTable("toggleView");
		}
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/game/admin/usersTasksItem/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#usersTasksItemTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function edit(id){//没有权限时，不显示确定按钮
	  ...
	  jp.openDialog('编辑用户任务', "${ctx}/game/admin/usersTasksItem/form?id=" + id,'800px', '500px', $('#usersTasksItemTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="game:admin:usersTasksItem:edit">
	  jp.openDialogView('查看用户任务', "${ctx}/game/admin/usersTasksItem/form?id=" + id,'800px', '500px', $('#usersTasksItemTable'));
