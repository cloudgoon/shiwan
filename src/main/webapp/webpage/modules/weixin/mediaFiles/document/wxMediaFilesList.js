...
$(document).ready(function() {
	$('#wxMediaFilesTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/weixin/wxMediaFiles/data",
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/weixin/wxMediaFiles/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该图文管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/weixin/wxMediaFiles/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/weixin/wxMediaFiles/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该图文管理记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/weixin/wxMediaFiles/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#wxMediaFilesTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/weixin/wxMediaFiles/form?id=" + getIdSelections();
  }
