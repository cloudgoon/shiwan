...
import com.jeeplus.modules.weixin.util.MyTrustManager;
import com.jeeplus.modules.weixin.util.RequestHandler;
import com.jeeplus.modules.weixin.util.TenpayUtil2;
...
public class WxApi {
    ...
    public static Map<String, String> getWSJSPayPara(String openId, String appId, String appsecret, String partnerkey,
        ...
        String strCurrentTime = TenpayUtil2.getWXTime(currentTime);
        long currentTimeLay30m = currentTime + 30 * 60 * 1000;// 30分钟后 【要求至少5分钟过期】
        String strCurrentTimeLay30m = TenpayUtil2.getWXTime(currentTimeLay30m);
        // 订 单 生 成 时 间 非必输
        ...
        try {
            ...
            if (null != retXmlStr && retXmlStr.contains("return_code")) {
                Map map = TenpayUtil2.doXMLParseByDom4j(retXmlStr);
                ...
                RequestHandler reqHandler_paySign = new RequestHandler(null,
                        null);
