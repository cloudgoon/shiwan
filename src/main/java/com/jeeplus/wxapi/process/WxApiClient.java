...
public class WxApiClient {
    ...
    public static JSONObject updateNewsMaterial2(JSONObject jsonObj, int index, String mediaId, WxAccount wxAccount)
        throws Exception {
        String accessToken = getAccessToken(wxAccount);
        JSONObject postObj = new JSONObject();
        postObj.put("media_id", mediaId);
    ...
    // 新增多图文永久素材
    public static JSONObject addMoreNewsMaterial(List<WxMsgNews> msgNewsList, WxAccount wxAccount)
        throws Exception {
        String accessToken = getAccessToken(wxAccount);
        JSONArray jsonArr = new JSONArray();
        for (WxMsgNews news : msgNewsList) {
